...
export default function StaffEquipment() {
  ...
  const fetchData = useCallback(async () => {
    try { const r = await api.get('/staff/equipments'); setItems(r.data.equipments); } catch {}
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await api.patch(`/staff/equipments/${editing}`, { equipment: form });
      } else {
        await api.post('/staff/equipments', { equipment: form });
      }
      setForm(EMPTY_FORM); setEditing(null); setShowForm(false); fetchData();
  ...
  return (
    <div>
      ...
      {items.length === 0 ? <p className="empty-state">No equipment yet.</p> : (
